...
class UserRoutes {
    static get routes() {
        ...
        const userController = new controller_1.UserController(finderUserService, finderUsersService, registerUserService, loginUserService, eliminatorUserService);
        router.post('/register', userController.register);
        router.post('/login', userController.login);
        router.use(auth_middleware_1.AuthMiddleware.protect);
        router.get('/', auth_middleware_1.AuthMiddleware.restrictTo(data_1.Role.SENDER), userController.findAllUsers);
        router.get('/:id', userController.findUserById);
        router.delete('/:id', auth_middleware_1.AuthMiddleware.restrictTo(data_1.Role.SENDER), userController.delete);
        return router;
    }
}
